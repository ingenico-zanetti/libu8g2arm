/*
 * camera2.c
 * interface for 5 Pi Camera with the hardware camera board attached to the GPIO:
 * - rotary encoder filtered, debounced (in hardware) and decoded (in software)
 * - Addressable RGB LEDs available (4 for each of the 4 encoder)
 * - fan speed control and monitoring
 * All those features are available through a dedicated UART link with a BluePill
 * connected to the hardware interface board through a FPC
 * The camera board also provide a hardware passthrough for the 7 pins required for the display with a dedicated 3V3 LDO
 * The original camera.c had all the debouncing and decoding implemented for the only supported encoder,
 * whose switch acted as a software powerdown switch on very long press.
 *
 * Based on:
